...
public class AnswerDraftPage {
	
	private DatabaseHelper databaseHelper;
	private QuestionPage questionPage; // For refreshing when returning to question page
	private int userId;
	private int questionId;
	private int answerIdToUpdate;
	...
	public AnswerDraftPage(DatabaseHelper databaseHelper, QuestionPage questionPage, int userId, int questionId) {
		this.databaseHelper = databaseHelper;
		this.questionPage = questionPage;
		this.userId = userId;
		this.questionId = questionId;
		this.answerIdToUpdate = -1;
	}
	
	public AnswerDraftPage(DatabaseHelper databaseHelper, QuestionPage questionPage, int userId, int questionId, int answerIdToUpdate) {
		this.databaseHelper = databaseHelper;
		this.questionPage = questionPage;
		this.userId = userId;
		this.questionId = questionId;
		this.answerIdToUpdate = answerIdToUpdate;
	}
	...
	public void show(Stage primaryStage) {
	    ...
	    // Create question display (has to be different than the previous question display because the author should not be able to have editing abilities in the answer draft page)
	    VBox questionDisplay = createQuestionDisplay(scrollLayout);
	    ...
	    // Set scroll layout
	    scrollLayout.getChildren().addAll(questionDisplay, new Separator(), contentEditor);
	    
	    layout.getChildren().addAll(layoutScrollPane, actionBar);
	    Scene answerDraftScene = new Scene(layout, 800, 400);

	    // Set the scene to primary stage
	    primaryStage.setScene(answerDraftScene);
	    primaryStage.setTitle("Answer Draft Page");
	}
	...
	public VBox createQuestionDisplay(VBox parent) {
		...
		// Title
		Label title = new Label(databaseHelper.getQuestionTitle(questionId));
	    ...
	    // Description
	    Label description = new Label(databaseHelper.getQuestionDescription(questionId));
	...
	public VBox createContentEditor(VBox parent) {
		...
		// Header
	    Label header = new Label("Answer Content");
	    ...
	    // Explain to write
	    Label description = new Label("Enter all the information someone would need to resolve their question.");
	    ...
	    contentField.setPromptText("Enter your answer here...");
	    ...
	    if (answerIdToUpdate != -1) {
	    	contentField.setText(databaseHelper.getAnswerContent(answerIdToUpdate));
	    }
	...
	public HBox createActionBar(VBox parent) {
		...
		postButton.setOnAction(a -> {
			Answer answer = new Answer(userId, questionId, contentField.getText());
			
			String errorMessage = Answer.validate(answer);
			if (errorMessage == "") {
				if (answerIdToUpdate != -1) {
					// Update answer
					databaseHelper.updateAnswerContent(answerIdToUpdate, answer.getContent());
